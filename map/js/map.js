...
function changeMap(siteKey, shadeClose) {
	...
	$.each(siteInfo, function(i, item) {
		html +=
			'<button type="button" class="layui-btn layui-btn-fluid" onclick ="loadPage(\''+i+'\');">'+item.name+'</button><hr>';
	});
	html +=
		'</div>';
	layer.open({
...
function setMyDistance(point) {
	...
	if (point) {
		...
		var sContent =
			...
			"<div style='margin:10px 0px;'>" + myValue +
			"</div>" +
...
function slideHeight() {
	...
	if (srcWidth <= 640) {
		if ($("#tabCon").hasClass('down')) {
			...
			$(".top .layui-icon-down").addClass("icon-xiangshang").removeClass("layui-icon-down");
		} else {
			$("#map-container").css("height", "42vh !important");
		$(".top .icon-xiangshang").addClass("layui-icon-down").removeClass("icon-xiangshang");
		}
	}
	resetMapNoChange();
	/* setTimeout(function() {
...
function resetMaps() {
	...
	if (tabCon > bottom)
		bottom = tabCon;
	$('#map-container').height($(window).height() - bottom + $('.searchBox').height() + $('#tab1').height());
	if (currpoint) {
...
function getOrgHTML(item, html) {
	...
	var html = "";
	html += '<li class="layui-menu-item-group layui-menu-item-up" lay-options="{type: \'group\'}">';
	html += '<div class="layui-menu-body-title">'+item.name+'<i class="layui-icon layui-icon-down"></i></div>';
	html += "<ul>";
	if (item.hallList && item.hallList.length > 0) {
		$.each(item.hallList, function(i, item) {
			html +=
				'<li class="gen" lay-value="'+item.id+'" lay-options="{id:\''+item.id+'\'}"><i class="layui-icon icon-left layui-icon-location"></i><a class="cursor">'+item.name+'</a></li>';
			//构造数据
			datas.push({
...
function attribute(e) {
	...
	resetMapNoChange();
	$("#datingList li.gen[lay-value='" + id + "']").parents(".layui-menu-item-up").children(
		".layui-menu-body-title").click();
	$("#datingList li.gen[lay-value='" + id + "']").parents("li.layui-menu-item-group").siblings(
		".layui-menu-item-down").children(".layui-menu-body-title").click();
...
function sconHTML(id) {
	...
	$.ajax({
		...
		success: function(res) {
			...
			if (res.success) {
				//读取个性化模板
				conHTML += setDetiles(res.result, id);
			} else {
				layer.msg("未搜索到结果！");
			}
		},
...
function getWorkHTML(item) {
	...
	if (item.itemList && item.itemList.length > 0) {
		$.each(item.itemList, function(i, item) {
			html += '<li class="gen" lay-options="{id: \'' + item.id +
				'\'}" ><i class="layui-icon icon-left layui-icon-list"></i><a class="cursor">' + item.name +
				'</a></li>';
			works.push(item);
		});
	} else if (item.childList && item.childList.length > 0) {
...
function getHotHTML(list) {
	...
	if (list && list.length > 0) {
		$.each(list, function(i, item) {
			conHTML +=
				'<li class="gen cursor" onclick="showYwInfo(\''+item.id+'\')" >'+(i+1)+'、'+ item.name+'<i title="'+item.dataIndex+'" class="layui-icon icon-map layui-icon-list"></i></li><hr>';
		});
	} else {
		conHTML += '<fieldset class="layui-elem-field layui-field-title" style="margin-top: 30px;">'
				+'				  <legend>暂无记录</legend>'
				+'				</fieldset>';
	}
	return conHTML;
...
function setIcon(id) {
	...
	for (i = 0; i < allOverlay.length; i++) {
		...
		if (allOverlay[i].options.hashCode != undefined && allOverlay[i].options.hashCode != "mypoint") {
			...
			if (datas[n].tag.id != id) {
				...
				//坐标点上显示的标题
				var title = "<p><b>" + datas[n].tag.name + "</b><a onclick=\"showMenu('" + id +
					"')\" ><img src='resource/marker/dh_icon.png'></a></p>";
				if (mod > 0)
					title = "<b>" + datas[n].tag.name + "</b><p>" + datas[n].tag.dataIndex.split('/')[mod] +
					"<a onclick=\"showMenu('" + id + "')\" ><img src='resource/marker/dh_icon.png'></a></p>";
				//console.log(datas[n].tag);
				openInfo(title, item);
				if (myPoint) {
					//有定位直线计算
					var distance = getDistance(item.lat, item.lng,
						myPoint.lat,
						myPoint.lng);
...
function searchResult(result) {
	...
	for (var i = 0; i < routes; i++) {
		...
		describeStr = "<p>总距离：" + Math.round(plan.getDistance()) + "公里，驾车时间约：" + formatSeconds(plan.getDuration()) +
			'</p>';
	...
	$.ajax({
		...
		success: function(res) {
			...
			if (res.success) {
				...
				+'					<hr>'
				+'					导航地址：'+item.hallAddress
				+'					<hr>'
				+'					<button type="button" class="layui-btn layui-btn-fluid" onclick ="openGoto(\''+id+'\','+mapType.baidu+');">百度地图</button><hr>'
				+'					<button type="button" class="layui-btn layui-btn-fluid" onclick ="openGoto(\''+id+'\','+mapType.amap+');">高德地图</button><hr>'
				+'					<button type="button" class="layui-btn layui-btn-fluid" onclick ="openGoto(\''+id+'\','+mapType.qq+');">腾讯地图</button>'
				+'				</div>'
				});
			} else {
				layer.msg("未搜索到结果！");
		...
		success: function(res) {
			...
			if (res.success) {
				...
				switch (type) {
					case mapType.baidu:
						url =
							'//api.map.baidu.com/marker?location='+item.lat+','+item.lng+'&title='+item.hallAddress+'&content='+item.name+'&output=html';
						break;
					case mapType.amap:
						url =
							'//uri.amap.com/marker?position='+bMapTransGMap(item.lng,item.lat)+'&name='+item.hallAddress+'&coordinate='+item.name+'&callnative=1';
						break;
					case mapType.qq:
						url =
							'//apis.map.qq.com/uri/v1/geocoder?coord='+bMapTransQQMap(item.lng,item.lat)+'&amp;name='+item.name+'&amp;addr='+item.hallAddress;
						break;
				}
...
function openPhoto(n) {
	...
	$.each(filePhoto, function(i, item) {
		if (item.materialFile.toLowerCase().indexOf(".jpg") >= 0 || item.materialFile.toLowerCase().indexOf(
				".png") >= 0) {
			if (n == i) {
				n = idx;
			}
...
function openCorrection(id, type) {
	...
	if (type == "service") {
		...
		layer.open({
			...
			shadeClose: true,
			shade: 0.4,
			content: 'correction-service.html?id=' + id + '&siteKey=' + $("#siteKey").val(),
		...
		layer.open({
			...
			shadeClose: true,
			shade: 0.4,
			content: 'correction-hall.html?id=' + id + '&siteKey=' + $("#siteKey").val(),
			success: function(layero, index) {
				var iframe = window['layui-layer-iframe' + index];
				// 信息窗口
